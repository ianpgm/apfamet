function run_hmmsearch(input_filenames, db_filename=Pkg.dir("apfamet")*"/db/Pfam-A.hmm", cores="1", sampleIDs=[], project_table_file = "project_table.txt", overwrite=false)
	...
	if length(input_filenames) != length(sampleIDs)
		print("Number of Sample IDs and filenames do not match")
		break
	end
	
	#Check to see whether hmmpress has been run, if not then run it
	if isfile(db_filename*".h3f")
	...
	if isfile(project_table_file)
		if overwrite==false
			print(project_table_file*" already exists, pick another name, delete the existing file, or specify overwrite=true")
			break
		end
	end
	...
	for filename in input_filenames
		...
		if isfile(output_filename)
			if overwrite==false
				print(filename*".hmmsearch_tblout already exists, pick another name, delete the existing hmmsearch output, or specify overwrite=true")
				break
			end
		end		
		run(`hmmsearch --tblout $output_filename --cpu $cores $db_fullpath $filename`)
	end
	...
	for (i, filename) in enumerate(input_filenames)
		write(output_table, sampleIDs[i]*"\t"*filename*"\n")
	end
